...
for estimator in estimators:
  xgb_e = XGBoostMultiClass(num_classes=num_classes, n_estimators=estimator)
  start_time_e = time.time()
  xgb_e.fit(Xtrain, ytrain)
  training_time = time.time()-start_time_e
  y_train_pred_e = xgb_e.predict(Xtrain)
  y_val_pred_e = xgb_e.predict(Xval)
...
learnrate=[0.01,0.02,0.1,0.2]
for lr in learnrate:
  xgb_l = XGBoostMultiClass(num_classes=num_classes, n_estimators=50,learning_rate=lr)
  start_time_l = time.time()
  xgb_l.fit(Xtrain, ytrain)
  training_time_l = time.time()-start_time_l
  y_train_pred_l = xgb_l.predict(Xtrain)
  y_val_pred_l = xgb_l.predict(Xval)
  train_acc_l = accuracy_score(ytrain, y_train_pred_l)
  val_acc_l = accuracy_score(yval, y_val_pred_l)
  train_f1_l = f1_score(ytrain, y_train_pred_l, average='weighted')
  val_f1_l = f1_score(yval, y_val_pred_l, average='weighted')
  print("----------------------------")
  print("XGBoost (Multi-class) + PCA RESULTS")
  print("----------------------------")
  print(f"Train Accuracy: {train_acc_l:.4f}")
  print(f"Validation Accuracy: {val_acc_l:.4f}")
  print(f"Train F1: {train_f1_l:.4f}")
  print(f"Validation F1: {val_f1_l:.4f}")
  print(f"Training time: {training_time_l:.2f} seconds")